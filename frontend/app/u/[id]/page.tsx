...
import {
  Hero, ArcRail, Rail, SkeletonRail, discoveryLabel, useLikes, CardActionsProvider,
} from '@/app/components'
...
export default function HomePage() {
  ...
  useEffect(() => {
    if (!userId) return
    setLoading(true)
    api.home(userId, debExplore, genre, anchor?.movie_id ?? 0)
      .then(setHome)
      .catch(() => setError('Failed to load recommendations.'))
      .finally(() => setLoading(false))
  }, [userId, debExplore, genre, anchor])
      ...
      <div className="min-h-full">
        ...
        <div className="border-b border-white/5 bg-zinc-950/50">
          <div className="mx-auto flex max-w-[1400px] flex-wrap items-center gap-x-8 gap-y-3 px-6 py-3">
            ...
            <div className="flex min-w-[300px] max-w-lg flex-1 items-center gap-4">
              <span className="text-xs font-medium uppercase tracking-wide text-zinc-500">Comfort</span>
              ...
              <span className="text-xs font-medium uppercase tracking-wide text-fuchsia-500/70">Discovery</span>
              <span className="w-[110px] shrink-0 rounded bg-white/10 px-2 py-0.5 text-center text-[11px] text-zinc-200">{discoveryLabel(explore)}</span>
            </div>
          </div>
        </div>
        ...
        <main className="mx-auto max-w-[1400px] px-6 pb-16 pt-10">
          {anchor && (
            <div className="mb-8 flex items-center gap-3 rounded-xl border border-fuchsia-500/20 bg-fuchsia-600/10 px-4 py-2.5 text-sm text-zinc-200">
              ...
              <span>More like <span className="font-semibold text-white">{anchor.title}</span> — the “Because you liked” rail now follows this pick.</span>
              <button onClick={() => setAnchor(null)} className="text-xs text-fuchsia-300 transition hover:text-fuchsia-200">clear</button>
            </div>
          )}
