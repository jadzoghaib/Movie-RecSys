      ...
      <div className="relative mx-auto flex max-w-[1400px] items-center gap-8 px-6 py-12 sm:py-16">
        {movie.poster_url && (
          <img src={movie.poster_url} alt={movie.title}
               ...
               className={`w-32 shrink-0 rounded-lg shadow-[0_30px_60px_rgba(0,0,0,.7)] ring-1 ring-white/10 transition-all duration-700 sm:w-[218px] ${onOpen ? 'cursor-pointer' : ''} ${loading ? 'scale-95 opacity-50 blur-sm' : 'opacity-100'}`} />
        )}
          ...
          <div className="flex flex-wrap gap-3">
            {onOpen && (
              <button onClick={() => onOpen(movie)}
                className="flex items-center gap-2 rounded-lg bg-white px-4 py-2.5 text-sm font-semibold text-zinc-900 transition hover:bg-zinc-200">
                <Play className="h-4 w-4 fill-current" /> Open details
              </button>
            )}
            {onMoreLikeThis && (
...
export function Rail({ rail }: { rail: RailT }) {
  ...
  const scroll = (dir: number) => ref.current?.scrollBy({ left: dir * 660, behavior: 'smooth' })
  return (
    <section className="group/rail mb-12">
      ...
      <div className="relative">
        <button onClick={() => scroll(-1)} aria-label="Scroll left"
          className="absolute -left-3 top-1/2 z-10 hidden h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full bg-black/70 text-zinc-200 opacity-0 transition group-hover/rail:flex group-hover/rail:opacity-100 hover:bg-red-600"><ChevronLeft className="h-5 w-5" /></button>
        <div ref={ref} className="flex gap-3 overflow-x-auto scroll-smooth pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {rail.items.map((m) => <div key={m.movie_id} className="w-[160px] shrink-0"><PosterCard movie={m} /></div>)}
        </div>
        <button onClick={() => scroll(1)} aria-label="Scroll right"
          className="absolute -right-3 top-1/2 z-10 hidden h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full bg-black/70 text-zinc-200 opacity-0 transition group-hover/rail:flex group-hover/rail:opacity-100 hover:bg-red-600"><ChevronRight className="h-5 w-5" /></button>
      </div>
    </section>
  )
}
...
export function PosterCard({ movie, badge }: { movie: Movie; badge?: string }) {
  ...
  return (
    <div ref={cardRef} className="group relative w-full rounded-lg outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2 focus-visible:ring-offset-[#0a0a0c]"
      role={onOpen ? 'button' : undefined} tabIndex={onOpen ? 0 : undefined}
      onClick={() => onOpen?.(movie)}
      onMouseEnter={show} onMouseLeave={hide} onFocus={show} onBlur={hide}
      onKeyDown={(e) => { if (onOpen && (e.key === 'Enter' || e.key === ' ')) { e.preventDefault(); onOpen(movie) } }}>
      <div className={`relative aspect-[2/3] w-full overflow-hidden rounded-lg bg-[#16161b] shadow-[0_2px_8px_rgba(0,0,0,.4)] ring-1 ring-white/10 transition-all duration-[350ms] [transition-timing-function:cubic-bezier(.22,1,.36,1)] group-hover:-translate-y-[7px] group-hover:shadow-[0_22px_46px_rgba(0,0,0,.7)] group-hover:ring-white/30 ${onOpen ? 'cursor-pointer' : ''}`}>
        {movie.poster_url ? (
          <img src={movie.poster_url} alt={movie.title} loading="lazy"
               className="h-full w-full object-cover transition duration-700 group-hover:scale-[1.07] group-hover:opacity-40" />
        ) : (
