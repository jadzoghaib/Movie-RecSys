...
export default function App() {
  ...
  return (
    <div className="min-h-full bg-[#0b0b0f] text-zinc-100">
      {/* Header */}
          ...
          <tbody>
            {metrics.map((m) => (
              <tr key={m.model} className="border-t border-white/5">
                ...
                {extra.map(([, k]) => <td key={k} className="px-4 py-2.5 text-right tabular-nums text-zinc-500">{(m as any)[k] != null ? (m as any)[k].toFixed(2) : '—'}</td>)}
              </tr>
            ))}
          </tbody>
        </table>
